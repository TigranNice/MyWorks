...
class Rotate_red_tank(pygame.sprite.Sprite):
    def __init__(self, red_tank_name, x, y, number, i=0, position=0, name=0):
        pygame.sprite.Sprite.__init__(self)
        self.number = number
        self.red_tank_name = red_tank_name
        self.name = name
        if self.number == '1':
            self.name = 1
        if self.number == '2':
        ...
        if self.number == '3':
            self.name = 3
        self.red_tank_name.convert_alpha()
        self.i = i
        self.x = x
        self.y = y
        self.position = position
        self.y = self.y + 30
        self.rect = self.red_tank_name.get_rect(center=(self.x, self.y)) # изменить

    def right_rotating(self, *args):
        if self.position == 0:
            self.red_tank_name = pygame.transform.rotate(self.red_tank_name, -90)
            self.position = 1
            if args[0] == 'up':
    ...
    def forward_movement(self, *args):
        ...
        if self.position == 1:
            if self.y - 40 <= 0:
                ...
                if self.name == 1:
                    self.y -= 0.2
                    self.rect = self.red_tank_name.get_rect(center=(self.x, self.y))
                if self.name == 2:
                    self.y -= 0.06
                    self.rect = self.red_tank_name.get_rect(center=(self.x, self.y))
                if self.name == 3:
